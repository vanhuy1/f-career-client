...
export default function VolumeSetting({
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const handleMouseMoveWithRef = (event: MouseEvent) =>
    ...
    <div className={cn('flex items-center gap-2')}>
      <div onClick={handleToggleMute} className="cursor-pointer">
        {volume > 0 ? (
          ...
          <Icon className={cn('filter-red-glow')} name="Mute" />
        )}
      </div>
      <div
        ref={containerRef}
        className={cn('flex cursor-pointer gap-1')}
