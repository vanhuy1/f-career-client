...
import { format, parse, addDays, isBefore, isToday, isFuture, isAfter } from 'date-fns';
import { Calendar as CalendarIcon, Bell, Trash2, Edit, Plus, RefreshCw, ExternalLink } from 'lucide-react';
...
export default function CalendarModal() {
    ...
    const calendarEvents = useSelector((state: RootState) => state.room.calendar.events);
    ...
    const [selectedEvent, setSelectedEvent] = useState<EventWithNotification | null>(null);
      ...
      if (typeof window !== 'undefined' && 'Notification' in window) {
        ...
        try {
          ...
          if (notificationTime > now) {
            ...
            const notifications = JSON.parse(localStorage.getItem('calendarNotifications') || '{}');
    ...
    // Render tabs
    const renderTabs = () => (
      <div className="flex border-b border-stone-700/50 mb-4">
        <button
          className={cn(
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-7 gap-1 mb-2">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
            <div key={day} className="text-xs text-center font-medium text-stone-400 py-1">
              {day}
            </div>
          ))}
        </div>
          ...
          {calendarDays.map((day, index) => (
            ...
            >
              ...
              {hasEventsOnDate(day.date) && (
                <span className="absolute bottom-1 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-green-400 rounded-full"></span>
              )}
            </button>
          ))}
        </div>
        ...
        <div className="mt-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <Button 
              ...
              className="bg-green-500/20 hover:bg-green-500/30 text-green-400 border border-green-500/50 flex items-center gap-1"
            >
              <Plus size={14} />
              <span>Add Event</span>
            </Button>
          ...
          {eventsForSelectedDate.length > 0 ? (
            <div className="space-y-2">
              {eventsForSelectedDate.map(event => (
                <div 
                  key={event.id}
                  className="p-3 rounded-md bg-stone-800/50 border border-stone-700/50 flex items-center"
                >
                  <div 
                    className="w-2 h-12 rounded-full mr-3 flex-shrink-0"
                    style={{ backgroundColor: (event as EventWithNotification).color || '#4ade80' }}
                  ></div>
                  <div className="flex-1">
                    <div className="text-white font-medium">{event.title}</div>
                    <div className="text-xs text-stone-400 flex items-center gap-4 mt-1">
                      {event.time && (
                      ...
                      {(event as EventWithNotification).notification && (
                        <span className="flex items-center">
                          ...
                          <span>{(event as EventWithNotification).notificationTime} min before</span>
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => handleEditEvent(event as EventWithNotification)}
                      className="p-1 rounded-full hover:bg-stone-700 text-stone-400 hover:text-white"
                    >
                      <Edit size={16} />
                    </button>
                    <button
    ...
    const renderEventForm = () => (
      <div className="p-4">
        <div className="space-y-4">
          <div>
            <Label htmlFor="title" className="text-stone-300">Event Title</Label>
            <Input
              id="title"
              name="title"
              ...
              className="bg-stone-800 border-stone-700/50 text-white mt-1"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="time" className="text-stone-300">Time (optional)</Label>
              <Input
                id="time"
                name="time"
                type="time"
          ...
          <div>
            <Label htmlFor="notes" className="text-stone-300">Notes (optional)</Label>
            <Textarea
              id="notes"
              name="notes"
              ...
              className="bg-stone-800 border-stone-700/50 text-white mt-1 h-20"
            />
          </div>
          
          <div>
            <Label htmlFor="color" className="text-stone-300">Color</Label>
            <div className="flex items-center gap-2 mt-1">
              {['#4ade80', '#60a5fa', '#f97316', '#a855f7', '#f43f5e'].map(color => (
                <button
                  key={color}
                  type="button"
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="notification" className="text-stone-300">Enable Notification</Label>
            </div>
            
            {eventForm.notification && (
              <div className="flex items-center gap-2">
                <Label htmlFor="notificationTime" className="text-stone-300 text-sm">Minutes before:</Label>
                <select
                  id="notificationTime"
                  name="notificationTime"
                  value={eventForm.notificationTime}
                  onChange={(e) => setEventForm(prev => ({ ...prev, notificationTime: Number(e.target.value) }))}
                  className="bg-stone-800 border border-stone-700/50 rounded-md text-white px-2 py-1 text-sm"
                >
          ...
          {isGoogleConnected && (
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) => handleSwitchChange('synced', checked)}
              />
              <Label htmlFor="synced" className="text-stone-300">Sync with Google Calendar</Label>
            </div>
          )}
          ...
          {isGoogleConnected ? (
            <div className="space-y-4">
              ...
              <p className="text-stone-300">
                Your events will be synced with Google Calendar when you create or update them if you enable sync for each event.
              </p>
              ...
              <div className="flex justify-center gap-4 mt-6">
                <Button 
                  ...
                  disabled={isSyncing}
                  className="bg-green-500/20 hover:bg-green-500/30 text-green-400 border border-green-500/50 flex items-center gap-2"
                >
                  {isSyncing ? (
                    <>
                ...
                <Button
                  ...
                  className="border border-stone-700/50 flex items-center gap-2"
                >
            ...
            <div className="space-y-4">
              <div className="text-xl font-medium text-white">Connect to Google Calendar</div>
              <p className="text-stone-300">
                Connect your Google Calendar to sync study room events and get notifications across all your devices.
              </p>
              
              <Button 
                onClick={connectToGoogle}
                disabled={isSyncing}
                className="bg-green-500/20 hover:bg-green-500/30 text-green-400 border border-green-500/50 flex items-center gap-2 mt-4"
              >
                {isSyncing ? (
                  <>
                    <RefreshCw size={16} className="animate-spin" />
