...
export const roomSlice = createSlice({
  ...
  reducers: {
    ...
    removeTodoItem: (state, action: PayloadAction<string>) => {
      state.todolist.items = state.todolist.items.filter(item => item.id !== action.payload);
    },
    ...
    updateCalendarEvent: (state, action: PayloadAction<CalendarEvent>) => {
      const index = state.calendar.events.findIndex(event => event.id === action.payload.id);
      if (index !== -1) {
        state.calendar.events[index] = action.payload;
      }
    },
    removeCalendarEvent: (state, action: PayloadAction<string>) => {
      state.calendar.events = state.calendar.events.filter(event => event.id !== action.payload);
    },
    startPomodoro: (state) => {
    ...
    completePomodoro: (state) => {
      ...
      state.pomodoro.dailyStats.totalWorkTime += state.pomodoro.settings.workDuration * 60;
      
      // Determine next phase
      if (state.pomodoro.completedPomodoros % state.pomodoro.settings.longBreakInterval === 0) {
        state.pomodoro.currentPhase = 'longBreak';
        state.pomodoro.timeRemaining = state.pomodoro.settings.longBreakDuration * 60;
      } else {
        state.pomodoro.currentPhase = 'break';
        state.pomodoro.timeRemaining = state.pomodoro.settings.breakDuration * 60;
      }
    },
    completeBreak: (state) => {
      state.pomodoro.currentPhase = 'work';
    ...
    updatePomodoroSettings: (state, action: PayloadAction<Partial<PomodoroSettings>>) => {
      state.pomodoro.settings = { ...state.pomodoro.settings, ...action.payload };
    ...
    toggleProgressVisibility: (state) => {
      state.pomodoro.ui.isProgressVisible = !state.pomodoro.ui.isProgressVisible;
    },
